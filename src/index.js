...
function draw() {
  ...
  for (var p = 0; p < points.length; p++) {
    ...
    c.beginPath();
    c.moveTo(
      points[p].v[0].x + Math.cos(a) * points[p].v[0].z,
      points[p].v[0].y + Math.sin(a) * points[p].v[0].z
    );
    c.lineTo(
      points[p].v[1].x - Math.cos(a) * points[p].v[1].z,
      points[p].v[1].y - Math.sin(a) * points[p].v[1].z
    );
    //c.closePath();
    c.stroke();
